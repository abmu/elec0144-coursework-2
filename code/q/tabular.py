...
class QLearning:
    ...
    def run(self, train: bool = False, verbose: bool = False) -> tuple[float, str]:
        ...
        if verbose:
            print(f'Iterations: {self.t - t_start} (t = {self.t}) | Pos: {next} | Epsilon: {self.epsilon:.3f}\n{self}')
        while not self.env.is_terminal(next):
            ...
            if verbose:
                ...
                print(f'Iterations: {self.t - t_start} (t = {self.t}) | Pos: {next} | Epsilon: {self.epsilon:.3f} | Prev. Action: {action} {act_str}\n{self}')
